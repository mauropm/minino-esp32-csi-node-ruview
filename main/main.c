/**
* @file main.c
* @brief Minino ESP32-C6 CSI Node — ADR-018 compliant firmware.
*
...
* aggregator over UDP.
*
* Target: Electronic Cats Minino (ESP32-C6)
...
/* Minino BSP for board-specific initialization */
...
void app_main(void)
{
    /* Initialize Minino BSP (board-specific peripherals) */
    uint32_t bsp_flags = MININO_BSP_I2C | MININO_BSP_SPI | MININO_BSP_UART;
#ifdef CONFIG_DISPLAY_ENABLE
    bsp_flags |= MININO_BSP_DISPLAY;
#endif
#ifdef CONFIG_SD_CARD_ENABLE
    bsp_flags |= MININO_BSP_SD_CARD;
#endif

    esp_err_t bsp_ret = minino_bsp_init(bsp_flags);
    if (bsp_ret != ESP_OK) {
        ESP_LOGW(TAG, "Minino BSP init failed: %s (continuing without board peripherals)",
                 esp_err_to_name(bsp_ret));
    }

    /* Blink LED to indicate startup */
    gpio_set_level(MININO_GPIO_LED, 1);
    ...
    ESP_LOGI(TAG, "Minino ESP32-C6 CSI Node (ADR-018) — v%s — Node ID: %d",
        app_desc->version, g_nvs_config.node_id);
    ESP_LOGI(TAG, "Target: %s (%s)", MININO_BOARD_NAME, MININO_MCU_TARGET);
