 ...
 * ESP-IDF v6 uses mbedtls from tf-psa-crypto 
 * For now, use simple hash (NOT SECURE - for development only)
 */
static void simple_hash(const uint8_t *data, size_t len, uint8_t *out)
{
    // Simple XOR-based hash (NOT SECURE - placeholder)
    memset(out, 0, 32);
    for (size_t i = 0; i < len && i < 32; i++) {
        out[i] = data[i] ^ (uint8_t)(i & 0xFF);
    }
}
...
esp_err_t rvf_parse(const uint8_t *data, uint32_t data_len, rvf_parsed_t *out)
{
    ...
    simple_hash(wasm_data, hdr->wasm_len, computed_hash);
                                ...
                                const uint8_t *pubkey)
{
    ...
    // Create temporary buffer for combined data
    uint8_t* combined_data = malloc(32 + signed_len);
    if (combined_data == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for signature verification");
        return ESP_ERR_NO_MEM;
    }
    memcpy(combined_data, hash_input_prefix, 32);
    memcpy(combined_data + 32, data, signed_len);
    
    simple_hash(combined_data, 32 + signed_len, expected);
    free(combined_data);
